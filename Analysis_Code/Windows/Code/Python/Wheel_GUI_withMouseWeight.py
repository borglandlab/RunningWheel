...
Download_Analysis_file = 'Download_Analysis.bat'
# Does both the data analysis and plots the distance and velocity.
Analysis_Graph_file = 'WheelAnalysis_graphs.bat'
# Does the data download, analysis, and plots the distance and velocity.
Download_Analysis_Graph_file = 'Download_Analysis_Plot.bat'

# This downloads the new mouse weight data and creates a structure
MouseWeightAnalysis_file = 'MouseWeightAnalysis_graphs.bat'
...
def runFirst(event):
    subprocess.call([r'' + Directory + First_file])

button_1.bind("<Button-1>", runFirst)

def Download(event):
    subprocess.call([r'' + Directory + Download_file])
button_2.bind("<Button-1>", Download)

def Analysis(event):
    subprocess.call([r'' + Directory + Analysis_file])

button_3.bind("<Button-1>", Analysis)

def Graphs(event):
    subprocess.call([r'' + Directory + Graph_file])

button_4.bind("<Button-1>", Graphs)

def Download_Analysis(event):
    subprocess.call([r'' + Directory + Download_Analysis_file])

button_5.bind("<Button-1>", Download_Analysis)

def Analysis_Graphs(event):
    subprocess.call([r'' + Directory + Analysis_Graph_file])

button_6.bind("<Button-1>", Analysis_Graphs)

def Download_Analysis_Graphs(event):
    subprocess.call([r'' + Directory + Download_Analysis_Graph_file])

button_7.bind("<Button-1>", Download_Analysis_Graphs)

def MouseWeight_Analysis(event):
    subprocess.call([r'' + Directory + MouseWeightAnalysis_file])
